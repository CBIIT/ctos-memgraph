...
if [[ "$MEMGRAPH_TYPE" == "memgraph" ]]; then
	if [[ "$MEMGRAPH_VERSION" == "3.11" ]]; then
		if [[ "$UBUNTU_VERSION" == "24.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph/v3.11.0/ubuntu-24.04/memgraph_3.11.0-1_amd64.deb
		elif [[ "$UBUNTU_VERSION" == "22.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph/v3.11.0/ubuntu-22.04/memgraph_3.11.0-1_amd64.deb
		else
			echo "Unable to download $MEMGRAPH_TYPE - $MEMGRAPH_VERSION for $UBUNTU_VERSION"
		fi
	elif [[ "$MEMGRAPH_VERSION" == "3.12" ]]; then
		if [[ "$UBUNTU_VERSION" == "24.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph/v3.12.0/ubuntu-24.04/memgraph_3.12.0-1_amd64.deb
		elif [[ "$UBUNTU_VERSION" == "22.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph/v3.12.0/ubuntu-22.04/memgraph_3.12.0-1_amd64.deb
		else
			echo "Unable to download $MEMGRAPH_TYPE - $MEMGRAPH_VERSION for $UBUNTU_VERSION"
		fi
	...
	if [[ "$MEMGRAPH_VERSION" == "3.11" ]]; then
		if [[ "$UBUNTU_VERSION" == "24.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph-mage/v3.11.0/ubuntu-24.04/memgraph-mage_3.11.0-1_amd64.deb
		elif [[ "$UBUNTU_VERSION" == "22.04" ]]; then
			DL_URL=https://download.memgraph.com/memgraph-mage/v3.11.0/ubuntu-22.04/memgraph-mage_3.11.0-1_amd64.deb
		else
			echo "Unable to download $MEMGRAPH_TYPE - $MEMGRAPH_VERSION for $UBUNTU_VERSION"
		fi
	elif [[ "$MEMGRAPH_VERSION" == "3.12" ]]; then
